...
void bookinfo(studbox* student, int m, bookbox* book, int q, studbookbox* studbk, int l) {
	...
	if (marker == 0) {
		...
		printf("Книги с ISBN '%s' нет ни у кого из студентов\n", isbn);
	}
}

//--------------------------------------------Поиск по автору-------------------------------------------------------------------

void findbooksurname(bookbox* book, int q) { // поиск автора
    char author[25];
    int flagbk = 0;
    int i = 0;
    ...
    for (int i = 0; i < q; i++) {
    	...
    	if (strcmp(book[i].author, author) == 0) { // если нашли студента
	...
	printf("Автора с фамилией '%s' не существует\n", author);
    }
}
...
void createbackupbk(bookbox* book, int q) { // бэкап книг в файл
...
bookbox* loadbackupbk(char* tablebook, int* q) { // загрузка книг из файла
    char backupname[20];
			...
			for (int j = 0; j < l; j++) {
				...
				if (strcmp(studbk[j].nu, nu) == 0) { 				// если нашли книгу у студента
					...
					for (int r = 0; r < q; r++) {
						...
						if (strcmp(book[r].isbn, studbk[j].isbn) == 0) { // вывод книги у студента
